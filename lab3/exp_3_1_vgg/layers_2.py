...
class ConvolutionalLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        self.input_pad[:, :, self.padding : self.padding + self.input.shape[2], self.padding : self.padding + self.input.shape[3]] = self.input # 扩充后的输入, 放到图片中间
        height_out = (height - self.kernel_size) // self.stride + 1 # 输出高度
        width_out = (width - self.kernel_size) // self.stride + 1 # 输出宽度
        self.output = np.zeros([self.input.shape[0], self.channel_out, height_out, width_out]) # 输出设置
...
class MaxPoolingLayer(object):
    ...
    def forward(self, input):
        ...
        height_out = (self.input.shape[2] - self.kernel_size) // self.stride + 1 # 输出高度
        width_out = (self.input.shape[3] - self.kernel_size) // self.stride + 1 # 输出宽度
