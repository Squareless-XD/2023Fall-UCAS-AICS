...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(43):
            if 'conv' in self.param_layer_name[idx]:
                ...
                bias = bias.reshape(-1)
                self.layers[self.param_layer_name[idx]].load_param(weight, bias) # 加载参数
    ...
    def load_image(self, image_dir):
        ...
        self.input_image = scipy.misc.imread(image_dir) # read image
        self.input_image = scipy.misc.imresize(self.input_image, [224, 224, 3]) # resize image
        self.input_image = np.array(self.input_image).astype(np.float32) # convert to float32
        self.input_image -= self.image_mean # subtract mean to get zero mean image
        self.input_image = np.reshape(self.input_image, [1] + list(self.input_image.shape)) # reshape to [1, height, width, channel]
    ...
    def forward(self):   # TODO：神经网络的前向传播
        ...
        for idx in range(len(self.param_layer_name)):
            ...
            current = self.layers[self.param_layer_name[idx]].forward(current) # 对每一层前向传播
