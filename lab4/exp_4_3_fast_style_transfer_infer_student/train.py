...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        #TODO: 返回残差运算的结果
        return nn.ReLU()(self.layer(x) + nn.Identity()(x)) # H(x) = F(x) + x
...
if __name__ == '__main__':
    ...
    s1, s2, s3, s4 = net.forward(image_style)
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            if i % 10 == 0:
                ...
                save_image(torch.cat((image_g, image_c), dim=3), './out/train/%d-%d.jpg' %(j, i))
        j += 1
